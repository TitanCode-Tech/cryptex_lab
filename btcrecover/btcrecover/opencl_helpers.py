    ...
    for i, platformNum in enumerate(pyopencl.get_platforms()):
        for device in platformNum.get_devices():
            ...
            if device.type & pyopencl.device_type.ACCELERATOR:  # always best
                if (
                    "oclgrind" not in device.name.lower()
                ):  # Some simulators present as an accelerator...
                    cur_score += 8
...
def init_opencl_contexts(loaded_wallet, openclDevice=0):
    ...
    wg = getattr(loaded_wallet, "chunksize", None)
    loaded_wallet.opencl_algo = opencl.opencl_algos(
        platform,
        debug,
        write_combined_file,
    ...
    loaded_wallet.opencl_algo_2 = opencl.opencl_algos(
        platform,
        debug,
        write_combined_file,
        inv_memory_density=1,
    ...
    loaded_wallet.opencl_algo_3 = opencl.opencl_algos(
        platform,
        debug,
        write_combined_file,
        inv_memory_density=1,
    ...
    if type(loaded_wallet) is btcrecover.btcrpass.WalletBlockchain:
        loaded_wallet.opencl_context_pbkdf2_sha1 = (
            loaded_wallet.opencl_algo.cl_pbkdf2_init(
                "sha1", len(loaded_wallet._salt_and_iv), 32
            )
        )
        return
    ...
    # Password recovery for blockchain.com wallet second password
        ...
        loaded_wallet.opencl_context_hash_iterations_sha256 = (
            loaded_wallet.opencl_algo.cl_hash_iterations_init("sha256")
        )
        return
    ...
    # Password recovery for Yoroi Cardano wallet
        ...
        loaded_wallet.opencl_context_pbkdf2_sha512 = (
            loaded_wallet.opencl_algo.cl_pbkdf2_init("sha512", 32, 32)
        )
        return

    # Password recovery for Bitcoin core wallet (or clones)
    elif type(loaded_wallet) is btcrecover.btcrpass.WalletBitcoinCore:
        loaded_wallet.opencl_context_hash_iterations_sha512 = (
            loaded_wallet.opencl_algo.cl_hash_iterations_init("sha512")
        )
        return
    ...
    # Password recovery for Electrum28 Wallet files
        ...
        loaded_wallet.opencl_context_pbkdf2_sha512 = (
            loaded_wallet.opencl_algo.cl_pbkdf2_init("sha512", len(b""), dklen)
        )
        return
    ...
    # Password recovery for BIP39 Passphrase or Electrum "Extra Words"
        ...
        loaded_wallet.opencl_context_pbkdf2_sha512 = (
            loaded_wallet.opencl_algo.cl_pbkdf2_saltlist_init(
                "sha512", len(loaded_wallet._mnemonic.encode()), dklen
            )
        )
        return
    ...
    # Password recovery for brainwallets
        ...
        if loaded_wallet.isWarpwallet:
            ...
            loaded_wallet.opencl_context_pbkdf2_sha256 = (
                loaded_wallet.opencl_algo_3.cl_pbkdf2_init(
                    rtype="sha256", saltlen=len(loaded_wallet.salt) + 1, dklen=32
                )
            )
        return
    ...
    # Seed Recovery for Cardano Wallets
        ...
        loaded_wallet.opencl_context_pbkdf2_sha512_saltlist = (
            loaded_wallet.opencl_algo.cl_pbkdf2_saltlist_init("sha512", 64, dklen=96)
        )
        loaded_wallet.opencl_context_pbkdf2_sha512 = []
        for salt in loaded_wallet._derivation_salts:
            loaded_wallet.opencl_context_pbkdf2_sha512.append(
                loaded_wallet.opencl_algo.cl_pbkdf2_init(
                    "sha512", len(b"mnemonic" + salt), dklen
                )
            )
        return
    ...
    # Passphrase Recovery for Cardano Wallets
        ...
        loaded_wallet.opencl_context_pbkdf2_sha512 = (
            loaded_wallet.opencl_algo.cl_pbkdf2_init("sha512", 33, dklen=96)
        )
        return
